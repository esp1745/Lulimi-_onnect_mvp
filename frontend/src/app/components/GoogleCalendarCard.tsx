...
export default function GoogleCalendarCard({
  ...
  const fetchStatus = async () => {
    try {
      ...
      /* leave status null, card just won't render a state */
    }
  };
  ...
  useEffect(() => {
    ...
    if (calendarParam === "connected") {
      ...
      toast.error(
        "Calendar permission wasn't granted. On the Google consent screen, make sure the Calendar access checkbox is checked before clicking Continue, then try again."
      );
      ...
      <CardContent>
        {status?.connected ? (
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            <Button size="sm" disabled={connecting} onClick={handleConnect} className="shrink-0">
