...
export default function GoogleSignInButton({ role, onSuccess }: {
  ...
  useEffect(() => {
    ...
    const handleCredentialResponse = async (response: GoogleCredentialResponse) => {
      try {
        const { data } = await api.post('/api/auth/google/', { credential: response.credential, role })
    ...
    const renderButton = () => {
      ...
      window.google.accounts.id.renderButton(containerRef.current, { theme: 'outline', size: 'large', width: '100%', text: role ? 'signup_with' : 'signin_with' })
    }
