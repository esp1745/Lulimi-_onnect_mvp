              ...
              <form onSubmit={handleAdd} className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    ...
                    <select value={form.day_of_week} onChange={set('day_of_week')} className="w-full border rounded-md px-3 py-2 text-sm bg-white">
                      {DAYS.map((d, i) => <option key={d} value={i}>{d}</option>)}
                    </select>
                  </div>
                  <div className="space-y-1">
                    <label className="text-sm font-medium text-gray-700">Timezone</label>
                    <select value={form.timezone} onChange={set('timezone')} className="w-full border rounded-md px-3 py-2 text-sm bg-white">
                      {TIMEZONES.map((tz) => <option key={tz} value={tz}>{tz}</option>)}
                    </select>
                  </div>
                  <div className="space-y-1">
                    <label className="text-sm font-medium text-gray-700">Start time</label>
                    <input type="time" value={form.start_time} onChange={set('start_time')} className="w-full border rounded-md px-3 py-2 text-sm bg-white" required />
                  </div>
                  <div className="space-y-1">
                    <label className="text-sm font-medium text-gray-700">End time</label>
                    <input type="time" value={form.end_time} onChange={set('end_time')} className="w-full border rounded-md px-3 py-2 text-sm bg-white" required />
                  </div>
                </div>
                <div className="flex gap-3">
                  ...
                  <Button type="button" variant="outline" onClick={() => { setAdding(false); setForm(EMPTY_SLOT) }}>
                    Cancel
                  </Button>
                </div>
              </form>
                ...
                {slots.map((slot) => (
                  <div key={slot.id} className="flex items-center justify-between px-5 py-4 gap-4">
                    <div className="flex items-center gap-4">
                      <div className={`w-2 h-2 rounded-full shrink-0 ${slot.is_active ? 'bg-emerald-500' : 'bg-gray-300'}`} />
                      <div>
                        <p className="font-medium text-sm">{DAYS[slot.day_of_week]}</p>
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      <Badge
                        variant="outline"
                        className={`text-xs cursor-pointer ${slot.is_active ? 'border-emerald-300 text-emerald-700' : 'text-gray-400'}`}
